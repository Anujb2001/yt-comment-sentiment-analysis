...
def load_data(data_url: str) -> pd.DataFrame:
    ...
    try:
        ...
        logger.error('Unexpected error occrred while loading the data: %s', e)
        raise

def preprocess_data(df: pd.DataFrame) -> pd.DateOffset:
    """Preprocess the data by handling missiong values, duplicates, and empty strings."""
    try:
        ...
        logger.debug('Data prepocessiong completed: Missing values, duplicates, and empty strings removed.')
        return df
...
def save_data(train_data: pd.DataFrame, test_data: pd.DataFrame, data_path: str) -> None:
    """Save the train and test dataset, createing the raw folder if it doesn't exist"""
    try:
